...
public class ImageSelectorActivity extends AppCompatActivity {
     ...
     * @param folder
     */
    private void setFolder(Folder folder,int status) {
        if(folder != null && mAdapter != null && status == 1){
            mFolder = folder;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == ImageSelector.RESULT_CODE) {
            ...
            if (resultCode == RESULT_OK) {
                ...
                String substring = mCameraImagePath.substring(mCameraImagePath.lastIndexOf("/") + 1, mCameraImagePath.length());
                Image jpeg = new Image(mCameraImagePath, 123, substring, "jpg", savePictureUri);
                folder.addImage(jpeg,1);
                mFolder = folder;
                setFolder(folder,1);
                //images.add(new Image(path, time, name, mimeType, uri));
                //folder.addImage(images.get(0));
